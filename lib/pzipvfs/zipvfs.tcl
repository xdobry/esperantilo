# patched from vfs::zip
# no tolower conversion

package provide pvfs::zip 1.0
package require vfs
...
proc pvfs::zip::Execute {zipfile} {
    Mount $zipfile $zipfile
    source [file join $zipfile main.tcl]
}

proc pvfs::zip::Mount {zipfile local} {
    set fd [::pzip::open [::file normalize $zipfile]]
    vfs::filesystem mount $local [list ::pvfs::zip::handler $fd]
    # Register command to unmount
    vfs::RegisterMount $local [list ::pvfs::zip::Unmount $fd]
    return $fd
}

proc pvfs::zip::Unmount {fd local} {
    vfs::filesystem unmount $local
    ::pzip::_close $fd
}

proc pvfs::zip::handler {zipfd cmd root relative actualpath args} {
    #::pvfs::log [list $zipfd $cmd $root $relative $actualpath $args]
...
proc pvfs::zip::attributes {zipfd} { return [list "state"] }
proc pvfs::zip::state {zipfd args} {
...
proc pvfs::zip::matchindirectory {zipfd path actualpath pattern type} {
    #::pvfs::log [list matchindirectory $path $actualpath $pattern $type]

    # This call to zip::getdir handles empty patterns properly as asking
    # for the existence of a single file $path only
    set res [::pzip::getdir $zipfd $path $pattern]
    #::pvfs::log "got $res"
    if {![string length $pattern]} {
	if {![::pzip::exists $zipfd $path]} { return {} }
...
proc pvfs::zip::stat {zipfd name} {
    #::pvfs::log "stat $name"
    ::pzip::stat $zipfd $name sb
    #::pvfs::log [array get sb]
    array get sb
}

proc pvfs::zip::access {zipfd name mode} {
    #::pvfs::log "zip-access $name $mode"
    if {$mode & 2} {
	vfs::filesystem posixerror $::vfs::posix(EROFS)
    }
    # Readable, Exists and Executable are treated as 'exists'
    # Could we get more information from the archive?
    if {[::pzip::exists $zipfd $name]} {
	return 1
    } else {
	error "No such file"
    }
...
proc pvfs::zip::open {zipfd name mode permissions} {
    #::pvfs::log "open $name $mode $permissions"
...
proc pvfs::zip::createdirectory {zipfd name} {
    #::pvfs::log "createdirectory $name"
    vfs::filesystem posixerror $::vfs::posix(EROFS)
}

proc pvfs::zip::removedirectory {zipfd name recursive} {
    #::pvfs::log "removedirectory $name"
    vfs::filesystem posixerror $::vfs::posix(EROFS)
}

proc pvfs::zip::deletefile {zipfd name} {
    #::pvfs::log "deletefile $name"
    vfs::filesystem posixerror $::vfs::posix(EROFS)
}

proc pvfs::zip::fileattributes {zipfd name args} {
    #::pvfs::log "fileattributes $args"
    switch -- [llength $args] {
	0 {
	    # list strings
	    return [list]
...
proc pvfs::zip::utime {fd path actime mtime} {
    vfs::filesystem posixerror $::vfs::posix(EROFS)
}
...
proc pzip::Data {fd arr {varPtr ""} {verify 0}} {
    upvar 1 $arr sb
...
proc pzip::open {path} {
    ...
    if {[catch {
	upvar #0 pzip::$fd cb
	upvar #0 pzip::$fd.toc toc

	fconfigure $fd -translation binary ;#-buffering none
	
	pzip::EndOfArchive $fd cb
...
proc pzip::exists {fd path} {
    #::vfs::log "$fd $path"
    if {$path == ""} {
	return 1
    } else {
	upvar #0 pzip::$fd.toc toc
	info exists toc($path)
    }
}

proc pzip::stat {fd path arr} {
    upvar #0 pzip::$fd.toc toc
    upvar 1 $arr sb
...
proc pzip::getdir {fd path {pat *}} {
    #::pvfs::log [list getdir $fd $path $pat]
    upvar #0 pzip::$fd.toc toc
	...
	foreach key [array names toc $path] {
		...
		#::pvfs::log "$sb(depth) vs $depth for $sb(name)"
	    }
	    unset sb
	}
	return $ret
